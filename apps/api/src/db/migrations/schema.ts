import { sqliteTable, AnySQLiteColumn, index, foreignKey, text, integer, uniqueIndex, real } from "drizzle-orm/sqlite-core"
...
(table) => [
	uniqueIndex("conversation_dm_key_unique").on(table.dmKey),
]);
...
(table) => [
	uniqueIndex("conversation_participant_conversation_user_unique").on(table.conversationId, table.userId),
...
(table) => [
	index("message_conversation_createdAt_idx").on(table.conversationId, table.createdAt),
...
export const news = sqliteTable("news", {
	...
	authorId: text("author_id").notNull().references(() => user.id, { onDelete: "cascade" } ),
	categoryId: text("category_id").references(() => newsCategory.id, { onDelete: "set null" } ),
	createdAt: integer("created_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
	updatedAt: integer("updated_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
...
(table) => [
	uniqueIndex("news_category_name_unique").on(table.name),
]);
...
export const event = sqliteTable("event", {
	...
	requiresRegistration: integer("requires_registration").default(true).notNull(),
	...
	attendeeListVisible: integer("attendee_list_visible").default(false).notNull(),
	createdAt: integer("created_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
	updatedAt: integer("updated_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
},
(table) => [
...
export const eventAttendee = sqliteTable("event_attendee", {
	memberId: text("member_id").primaryKey().notNull().references(() => eventMember.id, { onDelete: "cascade" } ),
	status: text().default("pending").notNull(),
	showInAttendeeList: integer("show_in_attendee_list").default(true).notNull(),
	createdAt: integer("created_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
	updatedAt: integer("updated_at").default(sql`(cast(unixepoch('subsecond') * 1000 as integer))`).notNull(),
},
(table) => [
	index("event_attendee_status_idx").on(table.status),
]);
...
(table) => [
	uniqueIndex("event_member_event_user_role_unique").on(table.eventId, table.userId, table.role),
...
(table) => [
	uniqueIndex("event_reminder_event_send_at_unique").on(table.eventId, table.sendAt),
...
export const eventSurvey = sqliteTable("event_survey", {
	id: text().primaryKey().notNull(),
	eventId: text("event_id").notNull().references(() => event.id, { onDelete: "cascade" } ),
...
(table) => [
	uniqueIndex("event_survey_event_audience_unique").on(table.eventId, table.audience),
